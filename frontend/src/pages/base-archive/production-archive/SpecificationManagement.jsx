...
import { specificationApi } from '../../../api/base-archive/production-archive/specification';
...
const SpecificationManagement = () => {
  ...
  const loadData = async (params = {}) => {
    setLoading(true);
    try {
      const response = await specificationApi.getSpecifications({
      ...
      if (response.data.success) {
        const { specifications, total, current_page } = response.data.data;
        
        // 为每行数据添加key
        const dataWithKeys = specifications.map((item, index) => ({
          ...item,
    ...
    try {
      ...
      if (index > -1) {
        ...
        if (item.id && !item.id.startsWith('temp_')) {
          // 更新现有记录
          response = await specificationApi.updateSpecification(item.id, row);
        } else {
          // 创建新记录
          response = await specificationApi.createSpecification(row);
        }
  ...
  const handleDelete = async (key) => {
    try {
      ...
      if (record.id && !record.id.startsWith('temp_')) {
        // 删除服务器记录
        const response = await specificationApi.deleteSpecification(record.id);
        if (response.data.success) {
      ...
      <td {...restProps}>
        {editing ? (
          <Form.Item
            ...
            rules={[
              {
                required: ['spec_name', 'length', 'width', 'roll'].includes(dataIndex),
                message: `请输入${title}!`,
              },
            ]}
          >
