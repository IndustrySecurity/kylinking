...
import { packageMethodApi } from '../../../api/base-archive/production-archive/packageMethod';

const { Title } = Typography;

const PackageMethodManagement = () => {
  ...
  const loadData = async (params = {}) => {
    setLoading(true);
    try {
      const response = await packageMethodApi.getPackageMethods({
    ...
    try {
      ...
      if (index > -1) {
        ...
        if (item.id && !item.id.startsWith('temp_')) {
          // 更新现有记录
          response = await packageMethodApi.updatePackageMethod(item.id, row);
        } else {
          // 创建新记录
      ...
      <td {...restProps}>
        {editing ? (
          <Form.Item
            ...
            rules={[
              {
                required: dataIndex === 'package_name',
                message: `请输入${title}!`,
              },
            ]}
          >
      ...
      <Card>
        <div style={{ marginBottom: 16 }}>
          <Row justify="space-between" align="middle">
            <Col>
              <Title level={4} style={{ margin: 0 }}>包装方式管理</Title>
            </Col>
            <Col>
              <Space>
                <Input
                  ref={searchInputRef}
                  placeholder="搜索包装方式名称、编码或描述"
                  value={searchText}
...
export default PackageMethodManagement; 