...
from app.services.business.inventory.material_transfer_service import MaterialTransferService
from app.models.business.inventory import (
    MaterialTransferOrder, MaterialTransferOrderDetail, Inventory
)
...
# 设置蓝图
bp = Blueprint('material_transfer', __name__)
...
def get_material_transfer_orders():
    ...
    try:
        ...
        logger.error(f"获取材料调拨单列表失败: {str(e)}")
...
def get_material_transfer_order(order_id):
    ...
    try:
        ...
        logger.error(f"获取材料调拨单详情失败: {str(e)}")
...
def update_material_transfer_order(order_id):
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"更新材料调拨单失败: {str(e)}")
...
def get_material_transfer_order_details(order_id):
    ...
    try:
        ...
        logger.error(f"获取材料调拨单明细失败: {str(e)}")
