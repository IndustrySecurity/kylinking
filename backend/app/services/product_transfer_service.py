...
class ProductTransferService:
    ...
    def _set_schema():
        ...
        if schema_name != 'public':
            current_app.logger.info(f"Setting search_path to {schema_name} in ProductTransferService")
            from app.extensions import db
    ...
    def create_transfer_order(data, created_by):
        ...
        try:
            # 设置schema搜索路径
            ProductTransferService._set_schema()
            ...
            # 查询仓库信息
            from_warehouse = db.session.query(Warehouse).filter_by(
                id=data['from_warehouse_id']
            ).first()
            to_warehouse = db.session.query(Warehouse).filter_by(
            ...
            # 创建调拨单
            transfer_order = ProductTransferOrder(
    ...
    @staticmethod
    def add_transfer_detail(transfer_order_id, product_data, created_by):
        """添加调拨明细"""
        try:
            # 验证调拨单存在
            transfer_order = db.session.query(ProductTransferOrder).filter_by(
            ...
            # 创建调拨明细
            detail = ProductTransferOrderDetail(
                ...
                for detail in transfer_order.details:
                    ...
                    if to_inventory:
                        ...
                        if to_inventory.current_quantity <= 0:
                            db.session.delete(to_inventory)
