...
class SalesOrderService:
    ...
    def get_sales_order_list(self, page: int = 1, page_size: int = 20,
        ...
        for order in orders:
            ...
            if order.contact_person_id:
                try:
                    contact = db.session.query(CustomerContact).filter_by(id=order.contact_person_id).first()
                    if contact:
            ...
            if order.salesperson_id:
                try:
                    salesperson = db.session.query(Employee).filter_by(id=order.salesperson_id).first()
                    if salesperson:
            ...
            if order.tracking_number:
                try:
                    merchandiser = db.session.query(Employee).filter_by(id=order.tracking_number).first()
                    if merchandiser:
            ...
            if order.tax_type_id:
                try:
                    tax_rate = db.session.query(TaxRate).filter_by(id=order.tax_type_id).first()
                    if tax_rate:
    ...
    def delete_sales_order(self, order_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # 但为了安全起见，我们手动删除
            db.session.query(SalesOrderDetail).filter_by(sales_order_id=order_id).delete()
            db.session.query(SalesOrderOtherFee).filter_by(sales_order_id=order_id).delete()
            db.session.query(SalesOrderMaterial).filter_by(sales_order_id=order_id).delete()
