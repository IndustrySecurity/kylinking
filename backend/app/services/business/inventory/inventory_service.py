...
class InventoryService(TenantAwareService):
    """
    库存管理服务类
    提供库存相关的业务逻辑操作
    """
    ...
    def get_inventory_by_id(self, inventory_id: str) -> Optional[Inventory]:
        """根据ID获取库存记录"""
        return self.get_session().query(Inventory).filter(Inventory.id == inventory_id).first()
    ...
    def get_inventory_by_warehouse_and_item(
        ...
        """根据仓库和物品获取库存记录"""
        query = self.get_session().query(Inventory).filter(Inventory.warehouse_id == warehouse_id)
    ...
    def create_inventory(
        ...
        **kwargs
        ...
        """创建库存记录"""
        inventory = self.create_with_tenant(
            Inventory,
    ...
    def generate_count_records(
        ...
        """为盘点计划生成盘点记录"""
        count_plan = self.get_session().query(InventoryCountPlan).filter(
    ...
    def record_count_result(
        ...
        """记录盘点结果"""
        count_record = self.get_session().query(InventoryCountRecord).filter(
    ...
    def get_inventory_aging_report(
        ...
        """获取库存账龄报表"""
        query = self.get_session().query(Inventory).filter(
            and_(
                Inventory.is_active == True,
        ...
        for inventory in inventories:
            # 获取最早的入库记录
            earliest_transaction = self.get_session().query(InventoryTransaction).filter(
                and_(
    ...
    def get_product_inventory(self, product_id: str) -> Dict[str, Any]:
        ...
        # 查询产品的所有库存记录
        inventories = self.get_session().query(Inventory).filter(
            and_(
    ...
    def get_inventory_transactions(
        ...
        transactions = query.order_by(InventoryTransaction.transaction_date.desc()).offset((page - 1) * page_size).limit(page_size).all()
    ...
    def get_inventory_statistics(
        ...
        # 基础库存统计
        inventory_query = self.get_session().query(Inventory).filter(Inventory.is_active == True)
        if warehouse_id:
            inventory_query = inventory_query.filter(Inventory.warehouse_id == warehouse_id)
        
        total_items = inventory_query.count()
        total_value = inventory_query.with_entities(func.sum(Inventory.total_cost)).scalar() or 0
        
        # 库存流水统计
        transaction_query = self.get_session().query(InventoryTransaction).filter(
            InventoryTransaction.transaction_date.between(start_date, end_date)
        )
        if warehouse_id:
            transaction_query = transaction_query.filter(InventoryTransaction.warehouse_id == warehouse_id)
        
        in_transactions = transaction_query.filter(InventoryTransaction.transaction_type.in_(['in', 'purchase_in', 'production_in'])).count()
        out_transactions = transaction_query.filter(InventoryTransaction.transaction_type.in_(['out', 'sales_out', 'production_out'])).count()
        
        # 低库存预警数
        low_stock_count = inventory_query.filter(Inventory.current_quantity <= Inventory.safety_stock).count()
    ...
    def get_inbound_order_list(
        ...
        query = self.get_session().query(InboundOrder).options(
    ...
    def get_inbound_order_by_id(self, order_id: str) -> Optional[InboundOrder]:
        """根据ID获取入库单"""
        from sqlalchemy.orm import joinedload
        order = self.get_session().query(InboundOrder).options(
            joinedload(InboundOrder.inbound_person),
            joinedload(InboundOrder.department)
    ...
    def get_inbound_order_details(self, order_id: str) -> List[Dict[str, Any]]:
        """获取入库单详情列表"""
        details = self.get_session().query(InboundOrderDetail).filter(
    ...
    def create_inbound_order(
        ...
        user = self.get_session().query(User).get(uuid.UUID(created_by))
    ...
    def update_inbound_order_detail(
        ...
        **update_data
        ...
        """更新入库单明细"""
        detail = self.get_session().query(InboundOrderDetail).filter(
...
def get_inventory_service(tenant_id: Optional[str] = None, schema_name: Optional[str] = None) -> InventoryService:
    """获取库存服务实例"""
    return InventoryService(tenant_id, schema_name) 