...
class ProductCountService(TenantAwareService):
    """成品盘点服务"""
    
    def create_count_plan(self, data: Dict[str, Any], created_by: str) -> Dict[str, Any]:
        """
        创建成品盘点计划
        ...
        try:
            ...
            count_plan = self.create_with_tenant(ProductCountPlan, **count_data)
            self.session.flush()
            
            # 自动获取该仓库的成品库存并生成盘点记录
            self._generate_count_records(count_plan.id, warehouse_id, created_by_uuid)
    ...
    def _generate_count_records(self, count_plan_id: uuid.UUID, warehouse_id: uuid.UUID, created_by: uuid.UUID):
        """
        为盘点计划生成盘点记录
        ...
        for inventory in inventories:
            # 获取产品信息
            product = self.session.query(Product).filter(Product.id == inventory.product_id).first()
            if not product:
                continue
    ...
    def get_count_plans(self, page: int = 1, page_size: int = 20, **filters) -> Dict[str, Any]:
        ...
        """
        try:
            query = self.session.query(ProductCountPlan)
    ...
    def get_count_plan(self, plan_id: str) -> Dict[str, Any]:
        """
        获取盘点计划详情
        ...
        """
        try:
            plan = self.session.query(ProductCountPlan).options(
    ...
    def get_count_records(self, plan_id: str, page: int = 1, page_size: int = 50) -> Dict[str, Any]:
        """
        获取盘点记录列表
        ...
        """
        try:
            query = self.session.query(ProductCountRecord).filter(
    ...
    def update_count_record(self, plan_id: str, record_id: str, data: Dict[str, Any], updated_by: str) -> Dict[str, Any]:
        """
        更新盘点记录
        ...
        """
        try:
            record = self.session.query(ProductCountRecord).filter(
                and_(
    ...
    def start_count_plan(self, plan_id: str, updated_by: str) -> Dict[str, Any]:
        """
        开始盘点计划
        ...
        """
        try:
            plan = self.session.query(ProductCountPlan).filter(ProductCountPlan.id == uuid.UUID(plan_id)).first()
            ...
            # 检查是否已有盘点记录，如果没有则生成
            existing_records = self.session.query(ProductCountRecord).filter(
                ProductCountRecord.count_plan_id == uuid.UUID(plan_id)
            ).count()
            
            if existing_records == 0:
                # 生成盘点记录
                self._generate_count_records(
    ...
    def complete_count_plan(self, plan_id: str, updated_by: str) -> Dict[str, Any]:
        """
        完成盘点计划
        ...
        try:
            ...
            # 检查是否所有记录都已盘点
            pending_count = self.session.query(ProductCountRecord).filter(
                and_(
                    ProductCountRecord.count_plan_id == uuid.UUID(plan_id),
    ...
    def adjust_inventory(self, plan_id: str, record_ids: List[str], updated_by: str) -> Dict[str, Any]:
        """
        根据盘点结果调整库存
        ...
        try:
            ...
            if not record_ids:
                records_to_adjust = self.session.query(ProductCountRecord).filter(
                    and_(
                        ProductCountRecord.count_plan_id == uuid.UUID(plan_id),
            ...
            for record_id in record_ids:
                ...
                # 获取库存记录
                inventory = self.session.query(Inventory).filter(Inventory.id == record.inventory_id).first()
    ...
    def get_warehouse_product_inventory(self, warehouse_id: str) -> List[Dict[str, Any]]:
        """
        获取仓库成品库存
    ...
    def delete_count_plan(self, plan_id: str, deleted_by: str) -> bool:
        """
        删除盘点计划
    ...
    def get_count_statistics(self, plan_id: str) -> Dict[str, Any]:
        """
        获取盘点统计信息
        ...
        try:
            # 基础统计
            total_records = self.session.query(ProductCountRecord).filter(
                ProductCountRecord.count_plan_id == uuid.UUID(plan_id)
            ).count()
            
            counted_records = self.session.query(ProductCountRecord).filter(
                and_(
                    ProductCountRecord.count_plan_id == uuid.UUID(plan_id),
            ...
            variance_records = self.session.query(ProductCountRecord).filter(
                and_(
                    ProductCountRecord.count_plan_id == uuid.UUID(plan_id),
                    ProductCountRecord.variance_quantity != 0,
                    ProductCountRecord.variance_quantity.isnot(None)
            ...
            adjusted_records = self.session.query(ProductCountRecord).filter(
                and_(
                    ProductCountRecord.count_plan_id == uuid.UUID(plan_id),
            ...
            # 差异统计
            variance_stats = self.session.query(
...
def get_product_count_service(tenant_id: str = None, schema_name: str = None) -> ProductCountService:
    """获取产品盘点服务实例"""
    return ProductCountService(tenant_id=tenant_id, schema_name=schema_name)