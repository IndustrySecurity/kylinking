...
class MaterialInboundService:
    ...
    def _set_schema():
        ...
        if schema_name != 'public':
            current_app.logger.info(f"Setting search_path to {schema_name} in MaterialInboundService")
            from app.extensions import db
    ...
    def _fill_department_info(self, orders):
        ...
        try:
            ...
            if department_uuids:
                # 查询部门信息
                departments = self.db.query(Department).filter(Department.id.in_(department_uuids)).all()
    ...
    def get_material_inbound_order_details(self, order_id: str) -> List[Dict[str, Any]]:
        """获取材料入库单明细"""
        self._set_schema()
        details = self.db.query(MaterialInboundOrderDetail).filter(
    ...
    def update_material_inbound_order_detail(
        ...
        **update_data
        ...
        """更新材料入库单明细"""
        self._set_schema()
        detail = self.db.query(MaterialInboundOrderDetail).filter(
...
def get_material_inbound_service(db: Session = None) -> MaterialInboundService:
    """获取材料入库服务实例"""
    if db is None:
        from app.extensions import db as default_db
        db = default_db.session
    return MaterialInboundService(db) 