...
class MaterialCategoryService:
    ...
    def _set_schema():
        """设置租户schema"""
        if hasattr(g, 'tenant_schema') and g.tenant_schema:
            db.session.execute(db.text(f"SET search_path TO {g.tenant_schema}, public"))
    
    @staticmethod
    def get_material_categories(page=1, per_page=20, search=None, material_type=None, is_enabled=None):
        """
        获取材料分类列表
        ...
        Returns:
            dict: 包含材料分类列表和分页信息
        """
        MaterialCategoryService._set_schema()
        ...
        # 获取单位信息
        units = {str(unit.id): unit.unit_name for unit in Unit.get_enabled_list()}
        ...
        for category in pagination.items:
            ...
            # 添加单位名称
            category_dict['base_unit_name'] = units.get(category_dict['base_unit_id'], '')
            category_dict['auxiliary_unit_name'] = units.get(category_dict['auxiliary_unit_id'], '')
            category_dict['sales_unit_name'] = units.get(category_dict['sales_unit_id'], '')
    ...
    @staticmethod
    def get_material_category_by_id(category_id):
        """
        根据ID获取材料分类
        ...
        """
        MaterialCategoryService._set_schema()
        category = MaterialCategory.get_by_id(category_id)
        if not category:
            return None
    ...
    def update_material_category(category_id, data):
        ...
        for unit_field in ['base_unit_id', 'auxiliary_unit_id', 'sales_unit_id']:
            if unit_field in data and data[unit_field]:
                unit = Unit.get_by_id(data[unit_field])
                if not unit:
                    raise ValueError(f'{unit_field}对应的单位不存在')
    ...
    @staticmethod
    def batch_update_material_categories(updates):
        """
        批量更新材料分类
