...
class OutboundOrderService:
    ...
    def _set_schema():
        ...
        if schema_name != 'public':
            current_app.logger.info(f"Setting search_path to {schema_name} in OutboundOrderService")
            from app.extensions import db
    ...
    def create_outbound_order(self, data: Dict[str, Any], created_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"创建出库单失败: {str(e)}")
    ...
    def update_outbound_order(self, order_id: str, data: Dict[str, Any], updated_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if details_data is not None:
                # 删除现有明细
                self.db.query(OutboundOrderDetail).filter(
    ...
    def execute_outbound_order(self, order_id: str, executed_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取出库单明细
            details = self.db.query(OutboundOrderDetail).filter(
                OutboundOrderDetail.outbound_order_id == order_id
            ).all()
    ...
    def get_outbound_order_details(self, order_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            self._set_schema()
            details = self.db.query(OutboundOrderDetail).filter(OutboundOrderDetail.outbound_order_id == order_id).all()
    ...
    def update_outbound_order_detail(self, order_id: str, detail_id: str, detail_data: Dict[str, Any], updated_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            detail = self.db.query(OutboundOrderDetail).filter(
...
def get_outbound_order_service(db: Session = None) -> OutboundOrderService:
    """获取出库单服务实例"""
    if db is None:
        from app.extensions import db as default_db
        return OutboundOrderService(default_db.session)
    return OutboundOrderService(db) 