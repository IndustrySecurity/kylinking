...
class MaterialTransferService:
    ...
    @staticmethod
    def create_transfer_order(data, created_by):
        """
        创建材料调拨单
        ...
        try:
            ...
            # 获取仓库信息
            from_warehouse = db.session.query(Warehouse).filter(Warehouse.id == from_warehouse_id).first()
            to_warehouse = db.session.query(Warehouse).filter(Warehouse.id == to_warehouse_id).first()
            ...
            # 创建调拨单
            transfer_order = MaterialTransferOrder(**transfer_data)
    ...
    @staticmethod
    def add_transfer_detail(transfer_order_id, material_data, created_by):
        """
        添加调拨明细
        ...
        try:
            ...
            # 获取调拨单
            transfer_order = MaterialTransferOrder.query.get(transfer_order_id)
            ...
            # 创建调拨明细
            detail = MaterialTransferOrderDetail(**detail_data)
    ...
    @staticmethod
    def confirm_transfer_order(transfer_order_id, confirmed_by):
        """
        确认调拨单
        ...
        try:
            ...
            for detail in transfer_order.details:
                inventory = db.session.query(Inventory).get(detail.from_inventory_id)
                if not inventory:
                    raise ValueError(f"材料 {detail.material_name} 库存不存在")
                
                if inventory.available_quantity < detail.transfer_quantity:
                    raise ValueError(f"材料 {detail.material_name} 可用库存不足")
                
                # 预留库存
                inventory.reserve_quantity(detail.transfer_quantity, confirmed_by)
                detail.detail_status = 'confirmed'
            
            transfer_order.status = 'confirmed'
            transfer_order.updated_by = uuid.UUID(confirmed_by)
    ...
    @staticmethod
    def execute_transfer_order(transfer_order_id, executed_by):
        """
        执行调拨单（出库）
    ...
    @staticmethod
    def receive_transfer_order(transfer_order_id, received_by):
        """
        接收调拨单（入库）
        ...
        try:
            ...
            for detail in transfer_order.details:
                # 检查调入仓库是否已有该材料库存
                to_inventory = db.session.query(Inventory).filter(
                    and_(
    ...
    @staticmethod
    def cancel_transfer_order(transfer_order_id, cancelled_by, reason=None):
        """
        取消调拨单
